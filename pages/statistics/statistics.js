...
Page({
  ...
  details: function (e) {
    let url = '/pages/statisticsdetails/statisticsdetails?id=' + e.currentTarget.dataset.id
    wx.navigateTo({
      url: url,
    })
  },
  ...
  refreshData: function () {
    ...
    wx.cloud.database().collection('user').where({
      ...
      .then(res => {
        var groupids = res.data[0].manage
        // this.setData({
              ...
              wx.cloud.database().collection('data').where({
                ...
                .then(res => {
                  res.data.forEach((value, index, array) => {
                    ...
                    //获取完成人数
                    let num = 0
                    for (let key in value.finished) {
                      if ((value.finished)[key] == true) {
                        num++
                      }
                    }
                    value.finishednum = num
                  })
                  group.data = res.data
